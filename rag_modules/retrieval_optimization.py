...
class RetrievalOptimizationModule:
    ...
    def metadata_filtered_search(self, query: str, filters: Dict[str, Any], top_k: int = 5) -> List[Document]:
        """
        带元数据过滤的检索 (Post-filtering 后置过滤)
        
        Args:
            query: 查询文本
            filters: 元数据过滤条件 (例如: {"category": "荤菜", "difficulty": ["简单", "中等"]})
            top_k: 返回结果数量
    ...
    def _rrf_rerank(self, vector_docs: List[Document], bm25_docs: List[Document], k: int = 60) -> List[Document]:
        """
        【核心算法】使用 RRF (Reciprocal Rank Fusion) 算法重排文档
        ...
        Returns:
            重排后的文档列表
        """
        doc_scores = {}  # 用于累加每个文档的 RRF 分数 (key: doc_id, value: score)
        doc_objects = {} # 用于保存文档对象本身 (key: doc_id, value: Document)
        ...
        # TODO 3: 根据 doc_scores 的 value (即最终分数) 对字典进行降序排序
        sorted_docs=sorted(doc_scores.items(),key=lambda x:x[1],reverse=True)
        # TODO 4: 根据排序后的结果，从 doc_objects 中取出对应的 Document 对象
        # 提示: 可以在这一步把最终的 RRF 分数写入 doc.metadata['rrf_score'] 中，方便后续调试
        reranked_docs=[]
